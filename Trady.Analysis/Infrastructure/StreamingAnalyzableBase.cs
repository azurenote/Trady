...
namespace Trady.Analysis.Infrastructure
{
    ...
    public abstract class StreamingAnalyzableBase<TInput, TMappedInput, TOutputToMap, TOutput> : IAnalyzable<TOutput>
    {
        private readonly bool _isTInputIOhlcvData, _isTOutputAnalyzableTick;

        internal protected readonly IList<TInput> _source;
        internal protected readonly IList<TMappedInput> _mappedInputs;
        private readonly IList<DateTimeOffset> _mappedDateTimes;
        internal protected readonly Func<TInput, TMappedInput> _inputMapper;

        protected StreamingAnalyzableBase(IList<TInput> inputs, Func<TInput, TMappedInput> inputMapper)
        {
            _source = inputs;
        ...
        public virtual void Add(TInput input)
        {
            _source.Add(input);
            _mappedInputs.Add(_inputMapper(input));

            if (_isTInputIOhlcvData)
            {
                _mappedDateTimes.Add((input as IOhlcv).DateTime);
            }
        }

        protected abstract TOutputToMap ComputeByIndexImpl(IList<TMappedInput> mappedInputs, int index);
    ...
    public abstract class StreamingAnalyzableBase<TInput, TOutput> : StreamingAnalyzableBase<TInput, TInput, TOutput, TOutput>
    {
        protected StreamingAnalyzableBase(IList<TInput> inputs) : base(inputs, i => i)
        {
        }
    }
}
